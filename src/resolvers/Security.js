...
export default {
  ...
  async osVersion(root, args, context) {
    ...
    let platformBracket = null;
    if ("platforms" in args.osVersion) {
      platformBracket = args.osVersion.platforms.find(
        (p) => p.name === platform
      );
    } else if (platform in args.osVersion) {
      platformBracket = args.osVersion[platform];
    }
