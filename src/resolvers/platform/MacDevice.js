...
const MacDevice = {
  ...
  async antivirus(root, args, context) {
    // Here check for XProtect antivirus
    const result = await kmd("antivirus", context);
    const XProtectVersion = parseInt(result.xprotectVersion, 10);

    const response = await applicationRunningFilter(args.providers, context);

    if (XProtectVersion) {
      response.push({ name: "XProtect" });
    }
